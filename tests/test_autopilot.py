...
def test_autotrader_uses_fallback_catalog_when_directory_unavailable(monkeypatch):
    ...
    try:
        ...
        assert state.last_skip_reason == "실시간 추천 데이터를 확보하지 못해 관망합니다."
        assert not state.analysis_markets
    finally:
        trader.stop()
...
def test_autotrader_uses_fallback_markets_when_recommendations_empty(monkeypatch):
    ...
    try:
        ...
        assert any("자동매매를 일시 중단" in entry.message for entry in state.logs)
        assert state.last_skip_reason == "실시간 추천 데이터를 확보하지 못해 관망합니다."
