...
def _make_market_data(count: int = 120, *, source: str = "upbit") -> MarketData:
    ...
    return MarketData(candles=candles, source=source)


def _wait_for_api_plan(client, attempts: int = 40, delay: float = 0.05):
    """Poll the auto-pilot status endpoint until a plan becomes available."""

    latest = None
    for _ in range(attempts):
        response = client.get("/trading/autopilot/status")
        assert response.status_code == 200
        payload = response.json()
        latest = payload
        if payload.get("last_plan"):
            return payload
        time.sleep(delay)
    return latest


def _wait_for_trader_plan(trader: AutoTrader, attempts: int = 40, delay: float = 0.05):
    """Poll the in-memory trader until a plan is generated."""

    snapshot = None
    for _ in range(attempts):
        snapshot = trader.status()
        if snapshot.last_plan is not None:
            return snapshot
        time.sleep(delay)
    return snapshot
...
def _make_recommendation_payload(market: str) -> MarketRecommendationPayload:
    return MarketRecommendationPayload(
        ...
        surge_probability_pct=37.0,
        crash_probability_pct=9.0,
        risk_reward_ratio=2.4,
        technical_confluence_score=82.0,
...
def _build_trader(
    ...
    if recommendation_scanner is None:
        default_candidates = ["KRW-BTC", "KRW-ETH"]

        def default_scanner(*_args, **_kwargs):
            return _make_recommendation_response(default_candidates)

        recommendation_scanner = default_scanner
...
def test_autopilot_request_accepts_blank_market_when_auto_select_enabled():
    payload = AutoPilotConfigRequest(auto_select_market=True, market="")
    assert payload.market == ""

    manual = AutoPilotConfigRequest(auto_select_market=False, market="")
    assert manual.market == DEFAULT_AUTOPILOT_MARKET


def test_autopilot_prefers_recommendations_when_market_blank():
    ...
    def scanner(*_args, **_kwargs):
        return SimpleNamespace(
            recommendations=[entry], analysis_source="synthetic", errors=[]
        )
    ...
    selected, candidates = trader._resolve_market(config)
    assert selected == "KRW-ETH"
    assert candidates[0] == "KRW-ETH"


def test_autopilot_start_handles_unexpected_exceptions():
    """AutoTrader.start should tolerate unexpected upstream failures."""

    def failing_candle_fetcher(**_kwargs):
        raise RuntimeError("unexpected upstream failure")

    trader, _ = _build_trader(candle_fetcher=failing_candle_fetcher)
    ...
    try:
        ...
        assert "unexpected upstream failure" in state.last_error
...
def test_autotrader_async_start_background_cycle():
    ...
    try:
        ...
        awaited = _wait_for_trader_plan(trader, attempts=80, delay=0.05)
        assert awaited is not None
        assert awaited.last_plan is not None, "백그라운드 사이클이 계획을 생성하지 못했습니다."
        assert trader.status().running is True
    finally:
        trader.stop()


def test_autopilot_relaxes_confidence_floor_for_nearby_signals():
    def low_confidence_plan(**kwargs) -> ai.AutoPilotOrderPlan:
        insight = kwargs["insight"]
        return ai.AutoPilotOrderPlan(
            market=insight.market,
            side="bid",
            ...
            confidence_pct=50.0,
            reasoning=["근접 신호 테스트"],
            monitoring=["신뢰도 완화 검증"],
        )

    trader, _ = _build_trader(autopilot_builder=low_confidence_plan)
    config = AutoTraderConfig(
        mode=OrderMode.PAPER,
        market="KRW-BTC",
        interval="minute60",
    ...
    trader.start(config, synchronous=True)
    try:
        state = trader.status()
        assert any("신뢰도 기준" in entry.message for entry in state.logs)
        assert state.effective_min_confidence <= 52.0
    finally:
        trader.stop()


def test_execute_paper_skips_when_price_invalid():
    trader, _ = _build_trader()
    trader._state.last_skip_reason = None
    ...
    plan = ai.AutoPilotOrderPlan(
        ...
        trailing_stop_pct=1.5,
        confidence_pct=75.0,
        reasoning=["유효하지 않은 시세 테스트"],
        monitoring=["데이터 정합성"],
    )

    execution = trader._execute_paper(plan=plan, position_pct=0.05, last_close=0.0)

    assert execution is None
    assert trader._state.last_skip_reason is not None
    assert "시세" in trader._state.last_skip_reason
...
def test_analysis_batch_rotates_when_not_truncated():
    trader, _ = _build_trader()
    candidates = ["KRW-BTC", "KRW-ETH", "KRW-XRP", "KRW-SOL", "KRW-ADA"]

    batch1, truncated1 = trader._select_analysis_batch(candidates)
    assert truncated1 is False
    assert batch1 == candidates

    batch2, truncated2 = trader._select_analysis_batch(candidates)
    assert truncated2 is False
    assert batch2[0] == "KRW-ETH"
    assert batch2[-1] == "KRW-BTC"
...
def test_autotrader_handles_cash_rounding_without_failure():
    ...
    trader = AutoTrader(
        ...
        time_provider=lambda: datetime.now(timezone.utc),
        notifier=lambda _message: True,
        recommendation_scanner=lambda *_args, **_kwargs: _make_recommendation_response([
            "KRW-BTC",
            "KRW-ETH",
        ]),
    )
...
def test_autotrader_uses_fallback_catalog_when_directory_unavailable(monkeypatch):
    ...
    try:
        ...
        assert state.last_recommendation_source == "fallback"
        assert state.last_skip_reason is None
        assert state.last_plan is not None
        assert state.analysis_markets
        assert any(
            "실시간 추천이 부족하지만 페이퍼 모드에서 안전하게 분석을 계속합니다." in entry.message
...
def test_autotrader_blocks_live_when_price_not_upbit():
    ...
    try:
        ...
        if state.last_price_source is not None:
            assert state.last_price_source in {"synthetic", "network-down"}
        assert state.last_skip_reason is not None
        assert any(
            keyword in state.last_skip_reason
            for keyword in ["업비트", "데이터 오류", "관망"]
        )
    finally:
        trader.stop()


def test_autotrader_allows_synthetic_prices_in_paper_mode():
    reset_market_state_for_tests()

    def synthetic_candle_fetcher(**kwargs):
        return _make_market_data(count=160, source="synthetic")

    trader, broker = _build_trader(candle_fetcher=synthetic_candle_fetcher)
    config = AutoTraderConfig(
        mode=OrderMode.PAPER,
        market="KRW-BTC",
        interval="minute60",
    ...
    try:
        assert state.running is True
        assert state.last_execution is not None, "합성 시세에서도 페이퍼 주문이 실행되지 않았습니다."
        assert state.last_price_source == "synthetic"
        assert state.last_skip_reason is None
        assert any("합성 시세" in log.message for log in state.logs)

        snapshot = broker.snapshot()
        assert snapshot.positions, "합성 시세 기반 주문이 포지션을 생성하지 못했습니다."
    finally:
        trader.stop()
...
def test_autotrader_recovers_when_recommendation_scanner_raises(monkeypatch):
    fallback_listing = MarketList(
        markets=[
            MarketInfo(
                market="KRW-BTC",
    ...
    def exploding_scanner(_base, _interval, _limit, _max_markets, _include_warnings):
        raise RuntimeError("scanner unavailable")

    trader, broker = _build_trader(recommendation_scanner=exploding_scanner)
    config = AutoTraderConfig(
        mode=OrderMode.PAPER,
        market="KRW-BTC",
        interval="minute60",
    ...
    try:
        assert state.recommendation_markets
        assert state.recommendation_market_count >= 5
        assert state.last_recommendation_source == "fallback"
        assert any("추천 종목 분석 실패" in entry.message for entry in state.logs)
        assert any(
            "실시간 추천이 부족하지만 페이퍼 모드에서 안전하게 분석을 계속합니다." in entry.message
            for entry in state.logs
        )
...
def test_autopilot_api_endpoints(monkeypatch):
    ...
    try:
        ...
        if not start_data.get("last_plan"):
            start_data = _wait_for_api_plan(client)
        ...
        status_data = _wait_for_api_plan(client)
...
def test_ai_copilot_falls_back_to_synthetic_data(monkeypatch):
    ...
    assert data["insight"]["price_source"] == "synthetic"
    assert any("실시간 업비트 시세가 아니라" in notice for notice in data["risk_notices"])
