...
def test_autopilot_forces_market_rotation_after_repeats():
    ...
    assert {first_market, second_market}.issubset({code for code in trader._state.recent_markets})
...
def test_autopilot_rotates_even_with_low_confidence_alternate():
    ...
    low_confidence_logs = [
        entry.message for entry in trader._state.logs if "대체 후보" in entry.message
    ]
    assert low_confidence_logs, "저신뢰 대체 후보 선택 로그가 기록되지 않았습니다."
