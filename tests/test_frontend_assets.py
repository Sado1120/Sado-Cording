...
def test_app_js_uses_cors_mode_for_api_requests():
    js = (PROJECT_ROOT / "frontend" / "app.js").read_text(encoding="utf-8")

    assert "mode = \"cors\"" in js, "requestApi는 교차 도메인 호출을 위해 CORS 모드를 설정해야 합니다"


def test_login_js_has_valid_syntax():
    node_path = shutil.which("node")
    if not node_path:
        pytest.skip("Node.js is not available for syntax validation")

    result = subprocess.run(
        [node_path, "--check", str(PROJECT_ROOT / "frontend" / "login.js")],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_register_js_has_valid_syntax():
    node_path = shutil.which("node")
    if not node_path:
        pytest.skip("Node.js is not available for syntax validation")

    result = subprocess.run(
        [node_path, "--check", str(PROJECT_ROOT / "frontend" / "register.js")],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_frontend_html_declares_utf8_and_korean_fonts():
    ...
    assert "logout-btn" in html, "Logout action should be exposed in the global navigation"
    required_metrics = {
        "metric-omega",
        "metric-kelly",
        "metric-streak-win",
    ...
    assert "analytics-summary" in html, "Analytics summary container should be present"
    assert "analytics-summary-status" in html, "Analytics summary status placeholder is missing"
    assert "analytics-summary-detail" in html, "Analytics summary detail placeholder is missing"
    assert "analytics-summary-updated" in html, "Analytics summary updated timestamp placeholder is missing"
    assert "analytics-summary-source" in html, "Analytics summary source badge should exist"
    ...
    assert "autopilot-start-top" in html, "상단 요약 섹션에서 오토파일럿을 바로 시작할 수 있어야 합니다."
    ...
    assert "chat-status-host" in html, "Chat host indicator should be rendered"
    ...
    assert "market-directory-status" in html, "Market explorer status block should be rendered"
    assert "selfcheck-summary" in html, "AI self-check summary should be rendered"
    assert "selfcheck-refresh" in html, "AI self-check refresh control must exist"
    ...
    assert "account-totp-qr-image" in html, "TOTP QR 이미지 자리표시자가 필요합니다."
    assert "account-totp-copy" in html, "TOTP 시크릿 복사 버튼이 있어야 합니다."
    assert "account-totp-download" in html, "TOTP QR 다운로드 버튼이 있어야 합니다."


def test_login_page_exposes_auth_controls():
    html = (PROJECT_ROOT / "frontend" / "login.html").read_text(encoding="utf-8")

    assert "charset=UTF-8" in html, "login.html must explicitly declare UTF-8 charset"
    assert "auth-login-form" in html, "Login page should render the authentication form"
    assert "auth-email" in html, "Email input must be present on the login page"
    assert "auth-password" in html, "Password input must be present on the login page"
    assert "auth-totp" in html, "TOTP input must be rendered for two-factor authentication"
    assert "login.js" in html, "Login page should load the dedicated authentication script"
    assert "register.html" in html, "Login page should link to the registration screen"


def test_register_page_exposes_onboarding_wizard():
    html = (PROJECT_ROOT / "frontend" / "register.html").read_text(encoding="utf-8")

    assert "charset=UTF-8" in html, "register.html must explicitly declare UTF-8 charset"
    assert "auth-wizard" in html, "Registration page should render the onboarding wizard"
    assert "register-form" in html, "Registration form must be present"
    assert "verify-form" in html, "Verification form must be rendered"
    assert "totp-step" in html, "TOTP 안내 패널이 필요합니다."
    assert "register.js" in html, "Registration page should load its dedicated script"
