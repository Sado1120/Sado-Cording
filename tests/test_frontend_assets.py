...
def test_login_page_exposes_auth_controls():
    ...
    assert "auth-register-form" in html, "Registration form should be available"
    assert "auth-verify-form" in html, "Verification form should be available"
    assert "auth-totp-card" in html, "TOTP 안내 화면이 포함되어야 합니다."
    ...
    assert "data-action=\"go-login\"" in html, "로그인 이동 버튼이 필요합니다."
    assert "totp-secret" in html, "보안 키 표시 요소가 필요합니다."
