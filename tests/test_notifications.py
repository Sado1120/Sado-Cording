...
from backend.app import post_chat_assistant, post_chat_notification
...
def test_chat_assistant_endpoint(monkeypatch):
    reset_chat_state()
    captured: dict[str, str] = {}

    def fake_run(request: AssistantRequest):
        captured["question"] = request.question
        return SimpleNamespace(
            question=request.question,
            answer="자동 생성 답변",
            next_steps=["차트 모니터링"],
            risk_notices=["변동성 확대 주의"],
        )
    ...
    monkeypatch.setattr(app_module, "run_ai_assistant", fake_run)
    monkeypatch.setattr(notifications, "notify_synology_chat", fake_notify)

    payload = ChatAssistantRequest(question="에이다 코인 매수 포지션 잡아줘", market="KRW-ADA")
    response = post_chat_assistant(payload)

    assert response.sent is True
    assert response.answer == "자동 생성 답변"
    assert "에이다" in captured["message"]
