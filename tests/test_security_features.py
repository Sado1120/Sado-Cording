...
from backend.security import auth_manager, hash_admin_password, AUTH_COOKIE_NAME
...
def test_extract_bearer_token_supports_fallback_headers():
    request = SimpleNamespace(headers={"authorization": "Bearer abc123"}, query_params={}, cookies={})
    assert _extract_bearer_token(request) == "abc123"

    request = SimpleNamespace(headers={"x-auth-token": "fallback-token"}, query_params={}, cookies={})
    assert _extract_bearer_token(request) == "fallback-token"

    request = SimpleNamespace(headers={}, query_params={"access_token": "query-token"}, cookies={})
    assert _extract_bearer_token(request) == "query-token"

    request = SimpleNamespace(headers={}, query_params={}, cookies={AUTH_COOKIE_NAME: "cookie-token"})
    assert _extract_bearer_token(request) == "cookie-token"

    request = SimpleNamespace(headers={}, query_params={}, cookies={})
...
def test_stateless_tokens_survive_restart(monkeypatch, tmp_path):
    from backend.security import AuthManager, AuthSettings

    store = CredentialStore(path=tmp_path / "security.json")
    settings = AuthSettings(token_ttl=2, token_secret="unit-test-secret")
    manager = AuthManager(store, settings)
    token = manager.issue_token("tester@example.com")
    assert manager.validate_token(token) is True

    # 새 프로세스가 떠도 같은 시크릿이면 토큰이 유효해야 한다.
    successor = AuthManager(store, settings)
    assert successor.validate_token(token) is True

    # 로그아웃 시 토큰이 즉시 무효화된다.
    successor.revoke_token(token)
    assert successor.validate_token(token) is False

    fresh = successor.issue_token("tester@example.com")
    assert successor.validate_token(fresh) is True

    future = time.time() + 10
    monkeypatch.setattr("backend.security.time", SimpleNamespace(time=lambda: future))
    try:
        assert successor.validate_token(fresh) is False
    finally:
        monkeypatch.setattr("backend.security.time", time)
