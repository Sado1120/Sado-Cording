...
def test_auth_can_be_disabled(monkeypatch):
    monkeypatch.setenv("AUTH_DISABLE", "1")
    import importlib
    from backend import security

    reloaded = importlib.reload(security)

    # ensure no exception when token missing
    reloaded.ensure_authenticated(None)
    token = reloaded.verify_login("anyone@example.com", "irrelevant", None)
    assert token
