...
def test_paper_endpoints_support_reset_mark_and_order():
    ...
    assert status_response.price_source in {"manual", "synthetic", "upbit", "upbit_stale"}
...
def test_get_paper_status_refreshes_market(monkeypatch):
    ...
    assert first.price_source in {"upbit", "upbit_alt", "upbit_stale"}
    assert second.price_source in {"upbit", "upbit_alt", "upbit_stale"}
...
def test_get_paper_status_auto_refreshes_stale_snapshot(monkeypatch):
    ...
    assert status.price_source in {"upbit", "upbit_alt", "upbit_stale"}
...
def test_submit_order_retries_after_execution_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.app.fetch_upbit_candles",
        lambda market="KRW-XRP", interval="minute1", count=1: DummyData(price=520.0),
    )
