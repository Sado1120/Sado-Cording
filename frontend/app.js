...
const setTotpRequirement = (required) => {
  ...
  if (authTotpInput) {
    if (totpRequired) {
      authTotpInput.setAttribute("required", "required");
    } else {
      authTotpInput.removeAttribute("required");
    }
  }
  if (authTotpField) {
    authTotpField.classList.toggle("auth-card__field--optional", !totpRequired);
  }
  if (authTotpLabel) {
    authTotpLabel.textContent = totpRequired
...
const fetchAuthStatus = async () => {
  try {
    ...
    // ignore and keep previous requirement
  }
};
...
const showAuthOverlay = (message) => {
  ...
  if (authUsernameInput && !authUsernameInput.value) {
    authUsernameInput.value = "sado0809";
  }
...
const startPeriodicTasks = () => {
  ...
  periodicTaskHandles.push(window.setInterval(updateAutopilotCountdown, 5_000));
  periodicTaskHandles.push(
    window.setInterval(() => {
      if (!isAuthenticated()) return;
...
const performLogin = async (username, password, totpCode) => {
  ...
  const url = joinApiUrl(base, "/auth/login");
  let response;
  try {
...
const authTotpInput = document.getElementById("auth-totp");
const authTotpField = document.querySelector("[data-auth-totp-field]");
const authTotpLabel = document.getElementById("auth-totp-label");
...
const initializeDashboardOnce = () => {
  ...
  setTotpRequirement(totpRequired);
  ...
  const handleAuthSubmit = async (event) => {
    ...
    const totpCode = authTotpInput ? authTotpInput.value.trim() : "";
    if (!username || !password) {
      setAuthError("아이디와 비밀번호를 입력하세요.");
      return;
    }
    if (totpRequired && (!totpCode || totpCode.length < 6)) {
      setAuthError("Google Authenticator 보안 코드를 입력하세요.");
      return;
    }

    setAuthError("");
    authSubmitBtn?.setAttribute("disabled", "disabled");
    try {
