...
const loadAuthToken = () => {
  ...
  try {
    const sessionValue = window.sessionStorage?.getItem(AUTH_TOKEN_KEY);
    if (sessionValue) {
      authToken = sessionValue;
      return authToken;
    }
  } catch (error) {
    console.warn("auth token session load failed", error);
  }
...
const clearAuthToken = () => {
  ...
  try {
    ...
    console.warn("auth token session clear failed", error);
  }
  try {
