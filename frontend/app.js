...
const refreshAppVersion = async () => {
  ...
  try {
    ...
    if (version) {
      ...
      appVersionEl.classList.remove("muted");
    } else {
      appVersionEl.textContent = "버전 확인 필요";
      appVersionEl.classList.add("muted");
    }
  } catch (error) {
    appVersionEl.textContent = "버전 확인 실패";
    appVersionEl.classList.add("muted");
  }
};
...
marketResultsToggleBtn?.addEventListener("click", () => {
  marketResultsExpanded = !marketResultsExpanded;
  renderMarketResults();
});

assistantRunBtn?.addEventListener("click", handleAssistant);
assistantClearBtn?.addEventListener("click", async () => {
  if (assistantClearBtn.hasAttribute("disabled")) {
    return;
  }
  assistantClearBtn.setAttribute("disabled", "true");
  try {
    await requestApi("/ai/assistant/history", { method: "DELETE" });
    assistantHistory = [];
    renderAssistantHistoryList(assistantHistory);
    clearAssistantHistoryStorage();
    setAssistantStatus("기록을 모두 삭제했습니다.", "neutral");
  } catch (error) {
    setAssistantStatus(`기록 삭제 실패: ${error.message}`, "danger");
  } finally {
    assistantClearBtn.removeAttribute("disabled");
  }
});
