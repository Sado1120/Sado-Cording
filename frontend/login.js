...
const buildApiCandidates = () => {
  ...
  const fromWindow = window.__SADO_API_BASE__ || "";
  const origin = window.location?.origin || "";
  const defaults = [stored, fromWindow, `${origin}/api`, DEFAULT_API_BASE, "http://127.0.0.1:8000"];
  defaults.forEach((item) => {
    const normalised = normaliseBase(item);
    if (normalised) {
...
const persistAuthToken = (token, ttlSeconds = DEFAULT_TOKEN_TTL_SECONDS) => {
  ...
  try {
    ...
    console.warn("auth token session persist failed", error);
  }
  try {
...
const requestApi = async (path, { method = "GET", body, headers = {} } = {}) => {
  ...
  for (const base of candidates) {
    ...
    try {
      ...
      lastError = new Error(
        `API 요청에 실패했습니다. ${error?.message ? `(${error.message})` : "네트워크 연결을 확인해주세요."}`.trim()
      );
      continue;
    }
    ...
    if (text) {
      try {
        ...
        if (!response.ok) {
          ...
          lastError = new Error(`서버 응답을 해석하지 못했습니다: ${preview}`);
          continue;
        }
  ...
  throw new Error("API 요청을 완료할 수 없습니다. 네트워크 구성을 확인해주세요.");
};
...
const fetchAuthStatus = async () => {
  try {
    ...
    if (payload?.auth_disabled) {
      setLoginHint("인증이 비활성화되어 바로 대시보드로 이동합니다.");
      persistAuthToken("guest", 12 * 60 * 60);
...
const performLogin = async (email, password, totpCode) => {
  ...
  persistAuthToken(payload.access_token, payload.expires_in);
  return payload;
};
