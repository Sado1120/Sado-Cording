...
const apiBaseInput = document.getElementById("register-api-base");
const apiStatusEl = document.getElementById("register-api-status");
...
const setApiStatus = (message, tone = "info") => {
  ...
  apiStatusEl.dataset.tone = tone;
  apiStatusEl.hidden = !text;
};

const applyApiBase = (value, { persist = true } = {}) => {
  const normalised = normaliseBase(value);
  if (!normalised) {
    setApiStatus("API 엔드포인트를 입력하세요.", "warning");
    return;
  }
  apiBase = normalised;
  if (persist) {
    persistApiBase(normalised);
  }
  if (apiBaseInput) {
    apiBaseInput.value = normalised;
  }
  setApiStatus(`API 엔드포인트 사용 중: ${normalised}`, "success");
};
...
const handleApiBaseChange = (event) => {
  applyApiBase(event?.target?.value || "");
};
...
const handleRegister = async (event) => {
  ...
  try {
    ...
    setApiStatus("API 연결이 확인되었습니다. 인증 코드를 발송했습니다.", "success");
    ...
    setApiStatus(error.message, "warning");
  }
};
...
const handleVerify = async (event) => {
  ...
  try {
    ...
    setApiStatus("이메일 인증이 완료되었습니다. 2단계 인증을 진행하세요.", "success");
...
const handleResend = async () => {
  ...
  try {
    ...
    setApiStatus("새 인증 코드를 재전송했습니다.", "success");
...
const init = () => {
  ...
  applyApiBase(apiBase, { persist: false });
  clearAuthErrorFlag();
  goToStep("register");
  updateVersion();
  ...
  if (apiBaseInput) {
    ...
    apiBaseInput.addEventListener("blur", handleApiBaseChange);
  }
