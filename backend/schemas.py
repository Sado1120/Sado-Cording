...
EmailString = constr(regex=EMAIL_REGEX, strip_whitespace=True, min_length=3, max_length=255)
...
class AuthProfileResponse(BaseModel):
    ...
    totp_hint: str
    totp_qr: str
    totp_rotated_at: Optional[datetime] = None
...
class SimulationResponse(BaseModel):
    ...
    ending_equity: float
    profit_krw: float
    price_source: Literal["upbit", "upbit_alt", "upbit_stale", "synthetic", "manual"] = "manual"
...
class MarketCandlesResponse(BaseModel):
    market: str
    interval: str
    source: Literal["upbit", "upbit_alt", "upbit_stale", "synthetic"]
...
class MarketRecommendationsResponse(BaseModel):
    ...
    analysis_source: Literal["upbit", "upbit_alt", "upbit_stale", "synthetic", "mixed"]
...
class MarketAIResponse(BaseModel):
    ...
    institutional_alerts: List[str]
    credible_sources: List[str]
    price_source: str = Field("unknown")
...
class AutoPilotConfigRequest(BaseModel):
    ...
    min_confidence_pct: float = Field(50.0, ge=0.0, le=100.0)
...
class AdaptiveProfilePayload(BaseModel):
    min_confidence_pct: float
    max_position_pct: float
    risk_appetite: float
...
class AutoPilotStatusResponse(BaseModel):
    ...
    adaptive_profile: Optional[AdaptiveProfilePayload] = None
