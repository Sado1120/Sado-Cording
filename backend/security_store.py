...
try:  # pragma: no cover - httpx optional at runtime
...
class CredentialStore:
    ...
    def provisioning_qr(self) -> str:
        """Return a data URI or hosted QR for authenticator setup."""

        uri = self.provisioning_uri()
        if not uri:
            return ""

        if segno is not None:  # pragma: no branch - prefer local generation when available
            try:
                qr = segno.make(uri)
                return qr.png_data_uri(scale=6)
            except Exception:
                pass

        encoded = quote(uri, safe="")
        fallback_url = (
            "https://chart.googleapis.com/chart?chs=220x220&cht=qr&chld=M|0&chl="
            f"{encoded}"
        )
        ...
        if httpx is not None:
            ...
            try:
                ...
                with urlopen(fallback_url, timeout=5.0) as handle:  # type: ignore[call-arg]
                    encoded_png = base64.b64encode(handle.read()).decode("ascii")
                    return f"data:image/png;base64,{encoded_png}"
            except Exception:
                pass
