...
class AutoTrader:
    ...
    def run_cycle(self) -> None:
        ...
        try:
            ...
            base_min_confidence = float(config.min_confidence_pct)
            ...
            for index, candidate in enumerate(analysis_candidates):
                ...
                if autopilot_plan.side == "ask" and not self._has_position(snapshot, autopilot_plan.market):
                ...
                elif autopilot_plan.confidence_pct < base_min_confidence:
                    if relaxation_active and autopilot_plan.confidence_pct >= relaxed_threshold:
                        ...
                        skip_reason = (
                            ...
                            f"기준 {base_min_confidence:.1f}% 미만입니다. 다음 추천을 확인합니다."
                        )
            ...
            with self._lock:
                if self._config:
                    ...
                    self._state.effective_min_confidence = (
                        relaxed_threshold if relaxation_active else base_min_confidence
                    )
    ...
    def _choose_actionable_candidate(
        ...
        for candidate in ordered[1:]:
            candidate_market = candidate.plan.market.upper()
            if candidate_market == last_market:
                continue
            if candidate.plan.confidence_pct >= top_confidence - rotation_tolerance:
                return candidate, True

        alt_threshold = max(5.0, min_confidence - _ALTERNATE_CONFIDENCE_MARGIN)
        if repeat_count >= _REPEAT_MARKET_ROTATION_THRESHOLD:
            alt_threshold = min(alt_threshold, _REPEAT_MARKET_CONFIDENCE_FLOOR)
        if repeat_count >= _REPEAT_MARKET_ROTATION_THRESHOLD:
            for candidate in ordered[1:]:
                candidate_market = candidate.plan.market.upper()
                if candidate_market == last_market:
                    continue
                if candidate.plan.confidence_pct >= alt_threshold:
                    self._append_log(
                        "info",
                        (
                            "동일 종목 반복을 방지하기 위해 "
                            f"{candidate_market} 후보를 선택했습니다."
