...
class AutoTrader:
    ...
    def _choose_actionable_candidate(
        ...
        if repeat_count >= _REPEAT_MARKET_ROTATION_THRESHOLD and alt_candidates:
            for candidate in alt_candidates:
                market_code = candidate.plan.market.upper()
                if market_code in recent_markets:
                    continue
    ...
    def _maybe_execute(
        ...
        *,
        ...
        if price_source not in {"upbit", "upbit_alt", "upbit_stale"}:
            if config.mode == OrderMode.LIVE:
                ...
                self._append_log(
                    "warning",
                    (
                        f"{plan.market} 시세 소스({price_source})가 합성 데이터라 관망합니다. "
                        "실시간 시세 복구 후 다시 시도합니다."
                    ),
                )
                self._set_skip_reason("실시간 시세 부재로 관망합니다.")
            return None
