...
class AutoTrader:
    ...
    # Lifecycle control
    # ------------------------------------------------------------------
    def start(self, config: AutoTraderConfig, *, synchronous: bool = False) -> AutoTraderState:
        """Apply configuration and spawn/refresh the background loop.

        When ``synchronous`` is ``True`` the first trading cycle is executed
        immediately before returning.  The default behaviour (``False``)
        defers the first cycle to the background worker so API handlers are not
        blocked by slow network calls when the auto-pilot starts.
        """
        ...
        if synchronous:
            ...
            with self._lock:
                # Ensure callers can see that the next cycle will be scheduled
                # immediately once the background thread is running.
                self._state.next_cycle_due_at = self._time_provider()
        self._ensure_thread()
        return self.status()
    ...
    def _resolve_market(self, config: AutoTraderConfig) -> Tuple[str, List[str]]:
        market_code = (config.market or "").upper()
        recommendations: List[str] = []
        source: Optional[str] = None
        candidate_markets: List[str] = [] if not market_code else [market_code]
