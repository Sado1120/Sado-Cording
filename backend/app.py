...
from .security import auth_manager, ensure_authenticated, verify_login
...
_PUBLIC_PATH_PREFIXES = ("/auth/login", "/docs", "/openapi", "/redoc")
...
def login(payload: LoginRequest) -> LoginResponse:
    token = verify_login(payload.username, payload.password, payload.totp_code)
...
def auth_profile() -> AuthProfileResponse:
    ...
    hint = f"{secret[:4]}····{secret[-4:]}" if len(secret) >= 8 else secret
    return AuthProfileResponse(
        username=snapshot.username,
        totp_enabled=snapshot.totp_enabled,
        totp_secret=secret,
        totp_uri=auth_manager.provisioning_uri(),
        totp_qr=auth_manager.provisioning_qr(),
        totp_hint=hint,
...
def update_password(payload: UpdatePasswordRequest) -> MessageResponse:
    snapshot = auth_manager.snapshot()
    if not auth_manager.authenticate_primary(snapshot.username, payload.current_password):
...
def reset_totp(payload: TotpResetRequest) -> TotpResetResponse:
    snapshot = auth_manager.snapshot()
    if not auth_manager.authenticate_primary(snapshot.username, payload.password):
        raise HTTPException(status_code=401, detail="현재 비밀번호가 올바르지 않습니다.")
