...
_RECOMMENDATION_CACHE: dict[
    Tuple[str, str, int, int, bool], Tuple[float, MarketRecommendationsResponse]
] = {}
...
def _evaluate_market_candidate(
    ...
    score = _safe_number(features.get("score"), lower=0.0)
    ...
    surge_pct = _safe_number(features.get("surge_probability_pct"), lower=0.0, upper=100.0)
    crash_pct = _safe_number(features.get("crash_probability_pct"), lower=0.0, upper=100.0)
    risk_reward_ratio = _safe_number(features.get("risk_reward_ratio"), lower=0.0)
    technical_score = _safe_number(features.get("technical_confluence_score"), lower=0.0, upper=100.0)
    technical_label = str(features.get("technical_confluence_label") or "중립")
    shock_risk_pct = _safe_number(features.get("shock_risk_pct"), lower=0.0, upper=100.0)
...
def _compute_market_recommendations(
    *,
    ...
    cache_key = (base_currency, interval, limit, max_markets, bool(include_warnings))
    ...
    top_recommendations: List[MarketRecommendationPayload] = deduped_scores[:limit]
