...
# Publicly accessible endpoints that should not enforce authentication.
# `/health` is kept open so the dashboard can render connectivity status even
# when a user is not yet logged in or when a stale token has been cleared.
_PUBLIC_PATHS = {"/meta/version", "/auth/logout", "/health"}
...
def _extract_bearer_token(request: Request) -> str:
    """Return an access token supplied via headers or query parameters."""
...
def login(payload: LoginRequest, response: Response) -> LoginResponse:
    ...
    ttl = int(auth_manager.settings.token_ttl)
    response.set_cookie(
        AUTH_COOKIE_NAME,
        token,
        max_age=ttl,
...
_AUTO_START_AUTOPILOT = _env_flag("AUTOPILOT_AUTO_START", True)


def _default_autopilot_config() -> AutoTraderConfig:
    capital_default = getattr(_paper_broker, "initial_cash", 20_000_000)
    capital = float(os.getenv("AUTOPILOT_DEFAULT_CAPITAL", capital_default))
    poll_interval = float(os.getenv("AUTOPILOT_DEFAULT_POLL_INTERVAL", 120.0))
    risk = float(os.getenv("AUTOPILOT_DEFAULT_RISK_APPETITE", 0.55))
    max_position = float(os.getenv("AUTOPILOT_DEFAULT_MAX_POSITION", 0.25))
    min_confidence = float(os.getenv("AUTOPILOT_DEFAULT_MIN_CONFIDENCE", 50.0))
    recommendation_base = os.getenv("AUTOPILOT_DEFAULT_BASE", "KRW").strip().upper() or "KRW"
    recommendation_interval = os.getenv(
        "AUTOPILOT_DEFAULT_RECOMMENDATION_INTERVAL", "minute60"
    ).strip() or "minute60"
    max_markets = int(os.getenv("AUTOPILOT_DEFAULT_MAX_MARKETS", 120))
    max_trades = int(os.getenv("AUTOPILOT_DEFAULT_MAX_TRADES", 1))
    interval = os.getenv("AUTOPILOT_DEFAULT_INTERVAL", "minute60").strip() or "minute60"
    include_portfolio = _env_flag("AUTOPILOT_DEFAULT_INCLUDE_PORTFOLIO", True)
    auto_select = _env_flag("AUTOPILOT_DEFAULT_AUTO_SELECT", True)
    include_warnings = _env_flag("AUTOPILOT_DEFAULT_INCLUDE_WARNINGS", False)
    market = os.getenv("AUTOPILOT_DEFAULT_MARKET", "KRW-BTC").strip().upper()
    if not market:
        market = "KRW-BTC"
    if not auto_select and not market:
        market = "KRW-BTC"
    ...
    return AutoTraderConfig(
        ...
        recommendation_interval=recommendation_interval,
        recommendation_max_markets=max_markets,
        recommendation_include_warnings=include_warnings,
        max_trades_per_cycle=max_trades,
    )


def _auto_start_autopilot_if_needed() -> AutoTraderState:
    state = _auto_trader.status()
    if not _AUTO_START_AUTOPILOT:
        return state
    if state.running:
        return state
    if state.last_cycle_started_at is not None:
        return state
    if getattr(state, "executions", None):
        return state

    config = state.config or _default_autopilot_config()
    try:
        return _auto_trader.start(config, synchronous=False)
    except Exception as exc:  # pragma: no cover - defensive guard
        log_exception("autopilot.autostart", exc)
        return _auto_trader.status()
...
def _autopilot_status_payload(state: AutoTraderState) -> AutoPilotStatusResponse:
    ...
    network_backoff = state.last_network_backoff or 0.0

    if not network_status:
        fallback_state = get_upbit_network_state()
        network_status = str(fallback_state.get("status") or "unknown")
        network_message = fallback_state.get("message") or network_message
        network_detail = fallback_state.get("detail") or network_detail
        network_checked_at = fallback_state.get("checked_at") or network_checked_at
        network_backoff = float(fallback_state.get("backoff_seconds_remaining") or network_backoff)

    price_source = (state.last_price_source or "").strip().lower()
    if state.running and price_source in {"synthetic", "upbit_alt", "upbit_stale"}:
        if not network_status or network_status.lower() in {"unknown", "down"}:
            network_status = "warning"
        if not network_message:
            network_message = "실시간 시세 대신 대체 데이터를 사용하고 있습니다."
        if not network_detail:
            network_detail = f"가격 소스: {price_source}" if price_source else None
        network_backoff = 0.0
    ...
    return AutoPilotStatusResponse(
        ...
        network_backoff_seconds=network_backoff,
...
def start_autopilot(payload: AutoPilotConfigRequest) -> AutoPilotStatusResponse:
    state = _auto_trader.start(_build_autopilot_config(payload), synchronous=False)
    response = _autopilot_status_payload(state)
    _push_chat_summary("autopilot-status", _summarise_autopilot_status_for_chat(response))
    log_event(
        "autopilot.start",
...
def get_market_intelligence(
    ...
    response = MarketAIResponse(
        ...
        price_source=str(getattr(data, "source", "unknown")),
    )
...
def run_ai_copilot(payload: CopilotRequest) -> CopilotResponse:
    ...
    candles, price_source = _load_candles_with_fallback(market, interval, 220)
    ...
    try:
        ...
        fallback_candles, price_source = _load_candles_with_fallback(market, interval, 260)
    ...
    if price_source != "upbit":
        risk_notices.insert(0, f"실시간 업비트 시세가 아니라 '{price_source}' 데이터를 사용 중입니다. 실거래는 피하세요.")
...
def _build_assistant_response(payload: AssistantRequest) -> AssistantResponse:
    ...
    if price_source != "upbit":
        synthesis.risk_notices.insert(
            0,
            f"실시간 업비트 시세가 아니라 '{price_source}' 데이터입니다. 실거래 전 확인이 필요합니다.",
        )
