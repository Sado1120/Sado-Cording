...
def evaluate_scalping_signal(
    ...
    bias = "neutral"
    label = "스캘핑 관망"
    if diff > 0 and slope > 0 and rsi_fast >= 54 and slow_slope >= 0:
        bias = "long"
        label = "스캘핑 매수 시그널"
    elif diff < 0 and slope < 0 and rsi_fast <= 46 and slow_slope <= 0:
        bias = "short"
        label = "스캘핑 매도 시그널"
    ...
    conviction = 38.0
    conviction += max(0.0, diff_ratio) * 2600
    conviction += max(0.0, slope_ratio) * 3200
    conviction += max(0.0, volume_ratio - 1.0) * 18.0
    if bias == "long":
        conviction += max(0.0, rsi_fast - 50.0) * 0.8
        conviction += max(0.0, momentum_pct) * 0.9
    elif bias == "short":
        conviction += max(0.0, 50.0 - rsi_fast) * 0.8
        conviction += max(0.0, -momentum_pct) * 0.9

    conviction = max(0.0, min(92.0, conviction))
    if bias == "neutral":
        conviction = min(conviction, 35.0)

    stop_loss_pct = max(0.35, min(8.0, atr_pct * 1.35 if atr_pct else 1.2))
    take_profit_pct = max(stop_loss_pct + 0.8, stop_loss_pct * 1.9)
    trailing_stop_pct = max(0.0, min(5.0, stop_loss_pct * 0.65))
...
def apply_scalping_signal(
    *,
    ...
    reasoning_note = f"스캘핑 강화: {scalping.label} · 신뢰도 {scalping.conviction_pct:.1f}%"

    merged_reasoning = _unique_preserve([reasoning_note, *plan.reasoning, *scalping.reasoning])
