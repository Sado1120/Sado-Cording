...
from .institutions import evaluate_institutional_alerts
...
class MarketAIMetrics:
    ...
    volatility_spike_score: float = 1.0
    momentum_acceleration_pct: float = 0.0
    shock_risk_pct: float = 0.0
    impulse_direction: str = "중립"
    impulse_strength_pct: float = 0.0
    regime_shift_probability: float = 0.0
    surge_probability_pct: float = 0.0
    crash_probability_pct: float = 0.0
    institutional_alert_level: float = 0.0
    institutional_alerts: List[str] = field(default_factory=list)
    credible_sources: List[str] = field(default_factory=list)
...
class TechnicalConfluence:
    """Aggregated indicator agreement for premium auto-trading workflows."""
...
class MarketAIInsight:
    ...
    technical_confluence: TechnicalConfluence
    institutional_alert_level: float
    institutional_alerts: List[str]
...
class ScalpingSignal:
    """Intraday trading cue derived from high-frequency candles."""
...
def _volatility_spike_score(candles: Sequence[Candle]) -> float:
    """Quantify volatility expansion by comparing recent ATR to its baseline."""

    if len(candles) < 40:
        return 1.0

    recent_atr = _average_true_range(candles[-40:])
    baseline_slice = candles[-160:-40] if len(candles) > 120 else candles[:-40]
    baseline_atr = _average_true_range(baseline_slice) if baseline_slice else 0.0
    ...
    score = recent_atr / baseline_atr if baseline_atr else 1.0
    return max(0.6, min(4.0, score))


def _momentum_acceleration(returns: Sequence[float]) -> float:
    """Capture second-derivative momentum pressure in percentage terms."""

    if len(returns) < 6:
        return 0.0

    recent = fmean(returns[-3:])
    prior = fmean(returns[-6:-3])
    return (recent - prior) * 100


def _shock_risk_pct(returns: Sequence[float]) -> float:
    """Estimate the size of the largest recent return swing."""

    if not returns:
        return 0.0

    window = returns[-12:] if len(returns) >= 12 else returns
    return max(abs(value) for value in window) * 100


def _impulse_signal(candles: Sequence[Candle], liquidity_pulse: float) -> Tuple[str, float]:
    """Detect sudden directional impulses backed by volume."""
    ...
    last_change = (closes[-1] - closes[-2]) / closes[-2] if closes[-2] else 0.0
    second_change = (closes[-2] - closes[-3]) / closes[-3] if closes[-3] else 0.0

    impulse_strength = abs(last_change) * 100 + max(0.0, liquidity_pulse - 1.0) * 40
    direction = "중립"

    threshold = 0.012 if liquidity_pulse >= 1.0 else 0.018
    if last_change > threshold and second_change > 0:
        direction = "상승"
    elif last_change < -threshold and second_change < 0:
        direction = "하락"

    return direction, impulse_strength


def _detect_candle_pattern(candles: Sequence[Candle]) -> Optional[str]:
    """Identify simple premium patterns for dashboard commentary."""
    ...
    if last_body > 0 and prev_body < 0 and last.close >= prev.open and last.open <= prev.close:
        return "강세 장악형"
    if last_body < 0 and prev_body > 0 and last.open >= prev.close and last.close <= prev.open:
        return "약세 장악형"
    ...
    body = abs(last_body)
    if body > 0:
        if lower_shadow >= body * 2 and upper_shadow <= body * 0.6:
            return "망치형 (반등 시그널)"
        if upper_shadow >= body * 2 and lower_shadow <= body * 0.6:
            return "교수형 (경고)"
    return None


def _technical_confluence(
    candles: Sequence[Candle], metrics: MarketAIMetrics, liquidity_pulse: float
) -> TechnicalConfluence:
    """Blend MACD/RSI/밴드/거래량 정보를 종합 점수로 환산."""
    ...
    bollinger = metrics.bollinger_bandwidth_pct

    score += max(-25.0, min(25.0, trend_strength * 1200))
    if trend_strength > 0.006:
        drivers.append("EMA 추세 강화")
    elif trend_strength < -0.006:
        drivers.append("EMA 약세 추세")

    macd_boost = max(-18.0, min(18.0, macd_hist * 5_000))
    score += macd_boost
    if macd_hist > 0:
        drivers.append("MACD 양수 가속")
    elif macd_hist < 0:
        drivers.append("MACD 음수 경고")

    if 45 <= rsi <= 60:
        score += 6.0
        drivers.append("RSI 중립권 상향")
    elif rsi < 35 or rsi > 70:
        score -= 8.0
        drivers.append("RSI 극단값 경계")

    score += max(-12.0, min(12.0, (breakout_probability - 0.5) * 40))
    if breakout_probability >= 0.6:
        drivers.append("돌파 확률 우위")

    squeeze_signal: Optional[str] = None
    if bollinger <= 5.5:
        squeeze_signal = "볼린저 밴드 스퀴즈"
        score += 4.0
    elif bollinger >= 12:
        squeeze_signal = "밴드 확장 진행"

    volume_confirmation = liquidity_pulse
    if liquidity_pulse >= 1.2:
        score += min(10.0, (liquidity_pulse - 1.0) * 20)
        drivers.append("거래량 급증 확인")
    elif liquidity_pulse <= 0.8:
        score -= min(12.0, (0.8 - liquidity_pulse) * 25)
        drivers.append("거래량 부족")
    ...
    if squeeze_signal:
        drivers.append(squeeze_signal)

    return TechnicalConfluence(
        score=score,
        label=label,
        drivers=drivers,
...
def _regime_shift_probability(
    *,
    acceleration_pct: float,
    volatility_spike: float,
    shock_risk_pct: float,
    consensus: Optional[TimeframeConsensus],
) -> float:
    """Estimate the chance that the market is transitioning to a new regime."""

    base = min(0.45, abs(acceleration_pct) / 80)
    volatility_component = max(0.0, (volatility_spike - 1.0) * 0.18)
    shock_component = max(0.0, shock_risk_pct - 4.0) * 0.02
    consensus_component = 0.0

    if consensus and consensus.dominant_trend != "중립":
        consensus_component = min(0.25, abs(consensus.agreement_pct - 50) / 200)

    probability = base + volatility_component + shock_component + consensus_component
...
def analyse_market(
    ...
    *,
    ...
    volatility_spike = _volatility_spike_score(candles)
    momentum_acceleration_pct = _momentum_acceleration(returns)
    shock_risk_pct = _shock_risk_pct(returns)
    impulse_direction, impulse_strength = _impulse_signal(candles, liquidity_pulse)
    regime_shift_probability = _regime_shift_probability(
        acceleration_pct=momentum_acceleration_pct,
    ...
    if any(keyword in action_lower for keyword in ("매수", "롱", "추세")):
        bias = "long"
    elif any(keyword in action_lower for keyword in ("매도", "숏", "헤지", "현금")):
        bias = "short"
    else:
        bias = "neutral"
    ...
    confidence += momentum_acceleration_pct * 0.4
    confidence += (volatility_spike - 1.0) * 12
    if hurst < 0.45:
        confidence -= 7
    ...
    summary = (
        ...
        f"스파이크 지수 {volatility_spike:.2f}x · 급등락 위험 {shock_risk_pct:.2f}% 입니다."
    )
    ...
    signals = [
        ...
        f"스파이크 {volatility_spike:.2f}x",
        f"모멘텀 가속 {momentum_acceleration_pct:.2f}%",
        f"충격 위험 {shock_risk_pct:.2f}%",
    ]
    if impulse_direction != "중립":
        signals.append(f"급등·급락 감지 {impulse_direction} ({impulse_strength:.1f})")

    stop_loss_pct = max(0.8, (volatility * 100) / 3)
    stop_loss_pct *= 1 + max(0.0, (0.6 - institutional_sentiment) * 0.8)
    if hurst < 0.45:
        stop_loss_pct *= 1.1
    if volatility_spike > 1.4:
        stop_loss_pct *= min(1.5, 0.9 + (volatility_spike - 1.0))
    ...
    if volatility_spike > 1.5 or shock_risk_pct > 6.0:
        position_size_pct *= 0.8
    position_size_pct = max(0.8, min(18.0, position_size_pct))

    bullish_pressure = 0.0
    bullish_pressure += max(0.0, (breakout_probability - 0.4) * 120)
    bullish_pressure += max(0.0, trend_strength) * 3800
    bullish_pressure += max(0.0, momentum_acceleration_pct) * 1.2
    bullish_pressure += max(0.0, (liquidity_pulse - 1.0) * 45)
    if impulse_direction == "상승":
        bullish_pressure += max(0.0, impulse_strength)

    bearish_pressure = 0.0
    bearish_pressure += max(0.0, (0.6 - breakout_probability) * 120)
    bearish_pressure += max(0.0, -trend_strength) * 3800
    bearish_pressure += max(0.0, -momentum_acceleration_pct) * 1.2
    bearish_pressure += max(0.0, shock_risk_pct * 4)
    if impulse_direction == "하락":
        bearish_pressure += max(0.0, impulse_strength)

    surge_probability_pct = max(0.0, min(100.0, 20.0 + bullish_pressure))
    crash_probability_pct = max(0.0, min(100.0, 15.0 + bearish_pressure))

    if surge_probability_pct >= 65.0 and bias == "long":
        take_profit_pct = max(take_profit_pct, stop_loss_pct * 2.4)
        trailing_stop_pct = max(trailing_stop_pct, stop_loss_pct * 0.95)
    if crash_probability_pct >= 60.0:
        position_size_pct *= 0.85
        if bias == "long":
            stop_loss_pct = min(stop_loss_pct * 1.2, stop_loss_pct + 4.5)
        else:
            trailing_stop_pct = max(trailing_stop_pct, stop_loss_pct)
    ...
    if volatility_spike > 1.4 or shock_risk_pct > 5.5:
        risk_notes.append("급등·급락 경보: 분할 진입과 타임스탑을 병행하세요.")
    if impulse_direction != "중립":
        risk_notes.append(
            f"{impulse_direction} 방향 급등락 신호가 감지되어 짧은 호흡의 대응이 필요합니다."
        )
    ...
    metrics = MarketAIMetrics(
        ...
        volatility_spike_score=volatility_spike,
        momentum_acceleration_pct=momentum_acceleration_pct,
        shock_risk_pct=shock_risk_pct,
        impulse_direction=impulse_direction,
        impulse_strength_pct=impulse_strength,
        regime_shift_probability=regime_shift_probability,
        surge_probability_pct=surge_probability_pct,
        crash_probability_pct=crash_probability_pct,
    )

    alert_level, institutional_alerts, credible_sources = evaluate_institutional_alerts(news_feed)
    metrics.institutional_alert_level = alert_level
    metrics.institutional_alerts = institutional_alerts
    metrics.credible_sources = credible_sources

    if alert_level >= 60:
        risk_notes.append("고위험 기관 경보: 포지션을 최소 25% 줄이고 방어적 대응을 유지하세요.")
        risk = replace(risk, position_size_pct=max(0.6, risk.position_size_pct * 0.75))
    elif alert_level >= 40:
        risk_notes.append("기관 경보가 감지되어 포지션을 경감하고 뉴스 흐름을 상시 점검하세요.")
        risk = replace(risk, position_size_pct=max(0.7, risk.position_size_pct * 0.9))

    if alert_level > 0:
        summary += f" 공신력 있는 기관 경보 {len(institutional_alerts)}건이 감지되었습니다."
        if credible_sources:
            summary += f" ({' · '.join(credible_sources[:3])})"
        signals.append(f"기관 경보 지수 {alert_level:.1f}")

    technical_confluence = _technical_confluence(
        candles=candles,
        metrics=metrics,
        liquidity_pulse=liquidity_pulse,
    )

    summary += f" 컨플루언스 점수는 {technical_confluence.score:.1f}점으로 {technical_confluence.label} 관점입니다."
    summary += (
        f" 급등 가능성 {surge_probability_pct:.1f}% · 급락 가능성 {crash_probability_pct:.1f}%로 "
        "모니터링이 필요합니다."
    )
    if technical_confluence.pattern:
        summary += f" 최근 패턴은 {technical_confluence.pattern} 입니다."
    ...
    signals.append(
        f"컨플루언스 {technical_confluence.score:.1f}점 ({technical_confluence.label})"
    )
    if technical_confluence.squeeze_signal:
        signals.append(technical_confluence.squeeze_signal)
    ...
    return MarketAIInsight(
        ...
        institutional_alert_level=alert_level,
...
def _derive_trade_bias(insight: MarketAIInsight) -> str:
    ...
    volatility_spike = float(getattr(metrics, "volatility_spike_score", 1.0) or 1.0)
    momentum_acceleration = float(getattr(metrics, "momentum_acceleration_pct", 0.0) or 0.0)
    shock_risk_pct = float(getattr(metrics, "shock_risk_pct", 0.0) or 0.0)
    impulse_direction = getattr(metrics, "impulse_direction", "중립") or "중립"
    impulse_strength = float(getattr(metrics, "impulse_strength_pct", 0.0) or 0.0)
    regime_shift_probability = float(getattr(metrics, "regime_shift_probability", 0.0) or 0.0)
    surge_probability_pct = float(getattr(metrics, "surge_probability_pct", 0.0) or 0.0)
    crash_probability_pct = float(getattr(metrics, "crash_probability_pct", 0.0) or 0.0)
    institutional_alert_level = float(getattr(metrics, "institutional_alert_level", 0.0) or 0.0)
...
def craft_autopilot_plan(
    *,
    ...
    metrics = insight.metrics
    confluence = getattr(insight, "technical_confluence", None)
    trend_strength = float(getattr(metrics, "trend_strength", 0.0) or 0.0)
    rsi = float(getattr(metrics, "rsi", 50.0) or 50.0)
    macd_hist = float(getattr(metrics, "macd_histogram", 0.0) or 0.0)
    ...
    bollinger_bandwidth_pct = float(getattr(metrics, "bollinger_bandwidth_pct", 0.0) or 0.0)
    volatility_component = max(0.0, volatility_spike - 1.0) * 0.45
    acceleration_component = max(0.0, abs(momentum_acceleration)) * 0.12
    bandwidth_component = max(0.0, (bollinger_bandwidth_pct - 6.0) / 28.0)
    tail_component = max(surge_probability_pct, crash_probability_pct) / 170.0
    scalping_profile = min(1.0, volatility_component + acceleration_component + bandwidth_component + tail_component)
    scalping_profile_pct = scalping_profile * 100.0
    ...
    if bias == "neutral" and impulse_direction in {"상승", "하락"}:
        bias = "long" if impulse_direction == "상승" else "short"
        side = "bid" if bias == "long" else "ask"
        contrarian_note = f"실시간 {impulse_direction} 급등락 신호 우선"
    ...
    if confluence:
        confluence_boost += max(-0.3, min(0.4, (confluence.score - 55.0) / 120))
        if confluence.volume_confirmation:
            confluence_boost += max(-0.25, min(0.35, (confluence.volume_confirmation - 1.0) * 0.6))
    scaled_position = (
        base_position
        * max(0.4, risk_appetite + 0.2)
        * breakout_boost
        * sentiment_boost
        * liquidity_boost
        * confluence_boost
    ...
    if volatility_spike > 1.5 or shock_risk_pct > 6.0:
        scaled_position *= 0.85
    position_size_pct = max(0.6, min(18.0, scaled_position))
    if institutional_alert_level >= 60:
        position_size_pct = max(0.6, position_size_pct * 0.75)
    elif institutional_alert_level >= 40:
        position_size_pct = max(0.7, position_size_pct * 0.9)
    ...
    if bias == "long":
        ...
        confidence_bonus += max(0.0, surge_probability_pct - 55.0) * 0.4
    elif bias == "short":
        confidence_bonus += max(0.0, -trend_strength) * 120.0
        if rsi <= 45:
        ...
        confidence_bonus += max(0.0, crash_probability_pct - 55.0) * 0.4

    confidence_bonus += momentum_acceleration * 0.8
    if confluence:
        confidence_bonus += max(-12.0, min(18.0, (confluence.score - 55.0) * 0.6))
    if impulse_direction != "중립":
        confidence_bonus += min(18.0, max(6.0, impulse_strength * 0.6))
    confidence_bonus += max(0.0, regime_shift_probability * 100 * 0.4)
    ...
    monitoring.append(
        f"급등락 위험 {shock_risk_pct:.2f}% · 스파이크 {volatility_spike:.2f}x · 레짐 전환 {regime_shift_probability * 100:.1f}%"
    )
    monitoring.append(
        f"급등 확률 {surge_probability_pct:.1f}% · 급락 위험 {crash_probability_pct:.1f}%"
    )
    if scalping_profile >= 0.2:
        monitoring.append(f"스캘핑 변동성 지수 {scalping_profile_pct:.0f}%")
    if institutional_alert_level > 0:
        alert_note = (
            f"기관 경보 {institutional_alert_level:.1f}점"
            + (f" · {institutional_alerts[0]}" if institutional_alerts else "")
        )
        monitoring.append(alert_note)
    ...
    if impulse_direction != "중립":
        reasoning.append(
            f"급등·급락 감지: {impulse_direction} ({impulse_strength:.1f}%), 모멘텀 가속 {momentum_acceleration:.2f}%"
        )
    else:
        reasoning.append(f"급등·급락 감지: 중립 (충격 위험 {shock_risk_pct:.2f}%)")

    if surge_probability_pct >= 65.0:
        reasoning.append(f"급등 가능성 {surge_probability_pct:.1f}%: 돌파 대비")
    if crash_probability_pct >= 60.0:
        reasoning.append(f"급락 위험 {crash_probability_pct:.1f}%: 자본 보호 모드")
    if institutional_alert_level >= 40:
        if credible_sources:
            reasoning.append(
                f"기관 경보 {institutional_alert_level:.1f}점 ({' · '.join(credible_sources[:2])})"
            )
        else:
            reasoning.append(f"기관 경보 {institutional_alert_level:.1f}점")

    if contrarian_note:
        reasoning.append(contrarian_note)

    if confluence and confluence.drivers:
        monitoring.extend(confluence.drivers[:3])
    ...
    if confluence and confluence.squeeze_signal and confluence.squeeze_signal not in monitoring:
        monitoring.append(confluence.squeeze_signal)
    ...
    if volatility_spike > 1.5:
        stop_loss_pct *= min(1.6, 0.95 + (volatility_spike - 1.0))
        take_profit_pct = max(take_profit_pct, stop_loss_pct * 1.8)
    if shock_risk_pct > 6.5:
        position_size_pct *= 0.85
        trailing_stop_pct = max(trailing_stop_pct, stop_loss_pct * 0.75)

    if scalping_profile >= 0.65 and bias != "neutral":
        position_size_pct = max(1.2, position_size_pct * 0.9)
        stop_loss_pct = max(0.45, stop_loss_pct * 0.75)
        take_profit_pct = max(stop_loss_pct * 1.4, min(take_profit_pct, stop_loss_pct * 1.9))
        trailing_stop_pct = max(trailing_stop_pct, stop_loss_pct * 0.6)
        order_type = "market"
        confidence_pct = max(confidence_pct, min(95.0, confidence_pct + scalping_profile * 6.0))
        reasoning.append(f"스캘핑 모드 활성화 (강도 {scalping_profile_pct:.0f}%)")
        monitoring.append("스캘핑 모드: 초단기 변동성 감시")
    elif scalping_profile >= 0.45:
        reasoning.append(f"스캘핑 감시 모드 (강도 {scalping_profile_pct:.0f}%)")
    ...
    if confluence:
        reasoning.append(
            f"컨플루언스 {confluence.label} · 점수 {confluence.score:.1f}"
        )
        if confluence.pattern and confluence.pattern not in reasoning:
            reasoning.append(f"캔들 패턴: {confluence.pattern}")
...
def evaluate_scalping_signal(
    candles: Sequence[Candle], *, interval: str = "minute3"
) -> Optional[ScalpingSignal]:
    """Derive a short-term trading cue from high-frequency candles."""

    if len(candles) < 24:
        return None

    closes = [candle.close for candle in candles if candle.close > 0]
    volumes = [max(0.0, candle.volume) for candle in candles]
    if len(closes) < 24:
        return None

    last_close = closes[-1]
    fast_now = _ema(closes[-20:], 5)
    fast_prev = _ema(closes[-21:-1], 5) if len(closes) > 21 else fast_now
    slow_now = _ema(closes[-34:], 13)
    slow_prev = _ema(closes[-35:-1], 13) if len(closes) > 35 else slow_now
    diff = fast_now - slow_now
    slope = fast_now - fast_prev
    slow_slope = slow_now - slow_prev
    rsi_fast = _rsi(closes, 7)
    atr_value = _average_true_range(candles[-50:], period=10)
    atr_pct = (atr_value / last_close * 100) if last_close else 0.0
    volatility_pct = max(0.05, atr_pct)

    if len(volumes) >= 12:
        recent_vol = fmean(volumes[-6:])
        base_slice = volumes[-30:-6] if len(volumes) >= 30 else volumes[:-6]
        base_vol = fmean(base_slice) if base_slice else recent_vol
        volume_ratio = recent_vol / base_vol if base_vol else 1.0
        if base_slice and len(base_slice) > 1:
            base_std = pstdev(base_slice)
            liquidity_z = (recent_vol - base_vol) / base_std if base_std else 0.0
        else:
            liquidity_z = 0.0
    else:
        volume_ratio = 1.0
    ...
    if len(closes) > 6 and closes[-6] > 0:
        momentum_pct = (closes[-1] - closes[-6]) / closes[-6] * 100.0
    else:
        momentum_pct = 0.0

    recent_window = candles[-60:] if len(candles) >= 60 else candles
    vwap_num = sum(c.close * max(0.0, c.volume) for c in recent_window)
    vwap_den = sum(max(0.0, c.volume) for c in recent_window)
    vwap = vwap_num / vwap_den if vwap_den else last_close
    vwap_gap_pct = ((last_close - vwap) / vwap * 100) if vwap else 0.0

    micro_returns: List[float] = []
    for idx in range(-12, 0):
        try:
            ...
            continue
        if prev_close > 0:
            micro_returns.append((current_close - prev_close) / prev_close)

    micro_trend_score = sum(micro_returns)
    micro_trend = "중립"
    if micro_trend_score > 0.004:
        micro_trend = "상승"
    elif micro_trend_score < -0.004:
        micro_trend = "하락"

    recent_high = max(c.high for c in recent_window)
    recent_low = min(c.low for c in recent_window)
    structure_note = "범위 관망"
    if recent_high > 0 and last_close >= recent_high * 0.998:
        structure_note = "고점 돌파"
    elif recent_low > 0 and last_close <= recent_low * 1.002:
        structure_note = "저점 붕괴"
    else:
        midpoint = (recent_high + recent_low) / 2 if recent_high and recent_low else last_close
        if midpoint:
            distance = abs(last_close - midpoint) / midpoint
            structure_note = "중앙 수렴" if distance < 0.003 else "박스권 유지"
    ...
    diff_ratio = diff / (abs(slow_now) + 1e-9)
    slope_ratio = slope / (abs(fast_prev) + 1e-9)

    conviction = 36.0
    conviction += max(0.0, diff_ratio) * 2500
    conviction += max(0.0, slope_ratio) * 3000
    conviction += max(0.0, volume_ratio - 1.0) * 14.0
    conviction += max(0.0, liquidity_z) * 6.0
    conviction += max(0.0, abs(vwap_gap_pct)) * 0.5
    if micro_trend == "상승":
        conviction += 4.0
    elif micro_trend == "하락":
        conviction += 4.0
    ...
    if bias == "long":
        conviction += max(0.0, rsi_fast - 50.0) * 0.7
        conviction += max(0.0, momentum_pct) * 0.85
        if vwap_gap_pct < -0.6:
            conviction -= 6.0
        if micro_trend == "하락":
            conviction -= 8.0
    elif bias == "short":
        conviction += max(0.0, 50.0 - rsi_fast) * 0.7
        conviction += max(0.0, -momentum_pct) * 0.85
        if vwap_gap_pct > 0.6:
            conviction -= 6.0
        if micro_trend == "상승":
    ...
    conviction = max(0.0, min(94.0, conviction))
    if bias == "neutral":
        conviction = min(conviction, 32.0)

    stop_loss_pct = max(0.35, min(7.5, atr_pct * 1.2 if atr_pct else 1.1))
    if structure_note == "고점 돌파":
        stop_loss_pct = max(stop_loss_pct, 0.5)
    take_profit_pct = max(stop_loss_pct + 0.7, stop_loss_pct * 1.8)
    trailing_stop_pct = max(0.0, min(4.8, stop_loss_pct * 0.6))

    reasoning = _unique_preserve(
        [
            f"EMA 5/13 격차 {diff_ratio * 100:.2f}%",
            f"EMA 기울기 {slope_ratio * 10000:.2f}",
            f"RSI7 {rsi_fast:.1f}",
            f"거래량 비율 {volume_ratio:.2f}x (z={liquidity_z:.2f})",
            f"VWAP 편차 {vwap_gap_pct:+.2f}%",
            f"최근 15분 수익률 {momentum_pct:.2f}%",
            f"마이크로 추세 {micro_trend}",
            f"구조 신호 {structure_note}",
        ]
    )
    ...
    return ScalpingSignal(
        ...
        liquidity_zscore=liquidity_z,
        structure_note=structure_note,
    )


def apply_scalping_signal(
    *,
    plan: AutoPilotOrderPlan,
    scalping: Optional[ScalpingSignal],
    risk_appetite: float,
    allow_short: bool,
) -> Tuple[AutoPilotOrderPlan, bool]:
    """Blend the scalping cue into the broader auto-trading plan."""

    if scalping is None or scalping.bias == "neutral":
        return plan, False

    if scalping.bias == "short" and plan.side == "flat" and not allow_short:
        return plan, False

    monitoring_note = (
        f"스캘핑 가드: 손절 {scalping.stop_loss_pct:.2f}% · 익절 {scalping.take_profit_pct:.2f}% "
        f"· 변동성 {scalping.volatility_pct:.2f}% · 거래량 {scalping.volume_ratio:.2f}x "
        f"· VWAP {scalping.vwap_gap_pct:+.2f}%"
    )
    reasoning_note = (
        f"스캘핑 강화: {scalping.label} · 신뢰도 {scalping.conviction_pct:.1f}% · "
        f"마이크로 추세 {scalping.micro_trend}"
    )

    structure_note = (
        f"구조: {scalping.structure_note} (유동성 z={scalping.liquidity_zscore:.2f})"
    )

    merged_reasoning = _unique_preserve(
        [reasoning_note, structure_note, *plan.reasoning, *scalping.reasoning]
    )
    merged_monitoring = _unique_preserve([monitoring_note, *plan.monitoring])

    new_confidence = max(plan.confidence_pct, scalping.conviction_pct)
    new_stop = min(plan.stop_loss_pct or scalping.stop_loss_pct, scalping.stop_loss_pct)
    new_take = max(plan.take_profit_pct or scalping.take_profit_pct, scalping.take_profit_pct)
    new_trailing = max(plan.trailing_stop_pct or 0.0, scalping.trailing_stop_pct)

    position_size_pct = plan.position_size_pct
    if position_size_pct <= 0:
        position_size_pct = min(20.0, max(3.0, (risk_appetite + 0.5) * 4.5))

    new_side = plan.side
    new_bias = plan.bias
    new_order_type = plan.order_type

    if plan.side == "flat":
        new_side = "bid" if scalping.bias == "long" else "ask"
        new_bias = scalping.bias
        new_order_type = "market" if scalping.conviction_pct >= 58.0 else "limit"
    elif scalping.bias != plan.bias and plan.bias not in {"neutral", scalping.bias}:
        if scalping.bias == "short" and not allow_short:
            return plan, False
