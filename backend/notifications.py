...
def _load_env_file_cache() -> Dict[str, str]:
    """Load key/value pairs from a .env-style file once per process."""
    ...
    # Allow overriding the search path for tests or custom deployments.
    candidate = os.getenv("SADO_ENV_FILE") or ".env"
    candidates = []
    if candidate:
...
def _reset_env_cache() -> None:
    """Reset cached .env values (primarily for testing)."""
...
def _resolve_webhook_url(override: Optional[str] = None) -> str:
    ...
    for key in ("SADO_CHAT_WEBHOOK", "SYNOLOGY_CHAT_WEBHOOK"):
        cleaned = _clean_webhook_url(cached.get(key))
        if cleaned:
            return cleaned
